...
архитектура:
web-larek создан по принципу ооп с использованием mvp архитектуры (model - view - presenter). Является видом MVC паттерна. 
Приложение, созданное на основе принципов MVP, функционирует через взаимодействие модели, вида и представителя следующим образом:

Вид создает интерфейс и наполняет его данными из модели.
Пользователь взаимодействует с интерфейсом, видя предоставленную информацию.
Вид перехватывает пользовательские события и передает их представителю.
Представитель, при необходимости, обрабатывает данные и передает их модели.
Модель выполняет необходимые операции и обновляет свои свойства.
Представитель получает обновленные данные модели и передает их обратно виду.
Вид обновляет интерфейс с новыми данными.
